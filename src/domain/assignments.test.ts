...
describe("assignmentsFor", () => {
  it("includes direct feature assignments and inherits the parent span for subtasks", () => {
    const features = [
      ...
      feature({
        ...
        duration: 3,
        resources: [],
        children: [{ id: "c1", title: "Sub", status: "planned", resources: ["A"], effort: 1, alloc: { A: 40 } }],
      }),
    ];
    const rows = assignmentsFor(features, "A");
    expect(rows).toEqual([
      { title: "Parent", effort: 1, start: 2, duration: 5, status: "planned", pct: 60 },
      { title: "Sub", effort: 1, parent: "Other", start: 10, duration: 3, status: "planned", pct: 40 },
    ]);
  });
});
