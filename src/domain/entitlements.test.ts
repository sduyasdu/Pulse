...
const delinquentSince = (t: number) => billing({ tier: "pro", status: "past_due", pastDueSince: t, updatedAt: t });

describe("entitlements (quota-only)", () => {
  it("absent billing doc resolves to Pro (the free default)", () => {
    expect(tierOf(null)).toBe("starter");
    expect(tierOf(undefined)).toBe("starter");
    expect(entitlementsFor(null)).toEqual(TIER_ENTITLEMENTS.starter);
    expect(entitlementsFor(null).maxPulses).toBe(3);
  });

  it("each tier exposes its quantity limits", () => {
    expect(entitlementsFor(billing({ tier: "starter" }))).toEqual({ maxEditors: 1, maxPulses: 3, maxCollaborators: 10, maxResourcesPerPulse: 20 });
    expect(entitlementsFor(billing({ tier: "pro" }))).toEqual({ maxEditors: null, maxPulses: 5, maxCollaborators: 20, maxResourcesPerPulse: 40 });
  ...
  it("a non-active status falls back to Pro (conservative, per PL4)", () => {
    for (const status of ["canceled", "incomplete"] as const) {
      expect(tierOf(billing({ tier: "business", status }))).toBe("starter");
    }
  ...
  it("delinquency grace: a past_due org keeps its paid tier for 15 days", () => {
    ...
    expect(tierOf(doc, T0)).toBe("pro"); // the moment the charge fails
    expect(tierOf(doc, T0 + 14 * DAY)).toBe("pro"); // day 14 — still paid
    expect(tierOf(doc, T0 + 15 * DAY - 1)).toBe("pro"); // last millisecond
    expect(tierOf(doc, T0 + 15 * DAY)).toBe("starter"); // window closes
    expect(tierOf(doc, T0 + 40 * DAY)).toBe("starter");
  });

  it("delinquency grace applies to quotas and seats, not just the tier label", () => {
    const doc = delinquentSince(T0);
    expect(entitlementsFor(doc, T0 + DAY)).toEqual(TIER_ENTITLEMENTS.pro);
    expect(entitlementsFor(doc, T0 + 20 * DAY)).toEqual(TIER_ENTITLEMENTS.starter);
    // Seats survive the window, then collapse to the single free Starter seat.
    expect(editorSeatLimit(billing({ tier: "pro", status: "past_due", pastDueSince: T0, seats: 6 }), T0 + DAY)).toBe(6);
    expect(editorSeatLimit(billing({ tier: "pro", status: "past_due", pastDueSince: T0, seats: 6 }), T0 + 20 * DAY)).toBe(1);
  });
  ...
  it("the grace clock runs from the first failed charge, not the latest retry", () => {
    ...
    const doc = billing({ tier: "pro", status: "past_due", pastDueSince: T0, updatedAt: T0 + 13 * DAY });
    expect(tierOf(doc, T0 + 16 * DAY)).toBe("starter");
  });

  it("a past_due doc with no pastDueSince falls back to updatedAt (pre-SF3 docs)", () => {
    const legacy = billing({ tier: "pro", status: "past_due", updatedAt: T0 });
    expect(tierOf(legacy, T0 + DAY)).toBe("pro");
    expect(tierOf(legacy, T0 + 16 * DAY)).toBe("starter");
  });
  ...
  it("editor seat limit: Pro fixed at 1; paid tiers bounded by purchased seats", () => {
    expect(editorSeatLimit(null)).toBe(1); // Starter default
    expect(editorSeatLimit(billing({ tier: "starter" }))).toBe(1);
    expect(editorSeatLimit(billing({ tier: "pro", seats: 4 }))).toBe(4);
