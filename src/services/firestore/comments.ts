...
export function subscribeAllComments(
  ...
  return onSnapshot(col(pulseId), (snap) => cb(map(snap.docs)), (err) => onError?.(err.message));
}

/** Comments for one target (a task id, or null for Pulse-level). */
export function subscribeCommentsFor(
  pulseId: string,
  targetId: string | null,
  cb: (comments: Comment[]) => void,
  onError?: (message: string) => void,
): () => void {
  return onSnapshot(query(col(pulseId), where("targetId", "==", targetId)), (snap) => cb(map(snap.docs)), (err) => onError?.(err.message));
}
