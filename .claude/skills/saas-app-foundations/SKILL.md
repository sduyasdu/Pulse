...
description: Use when starting a new multi-tenant SaaS web app, or when adding one of its four foundations to an existing one — architecture/stack choice, internationalization, billing with plan limits, or the mobile version. Front-loads the four decisions that are cheap on day one and expensive to retrofit, and carries the failure modes that pass every green check and still ship broken. Written from building Pulse (React + Firebase + Stripe, six languages, three tiers, mobile-first views).
---

# Foundations of a multi-tenant SaaS app

Four things are cheap before the first user and painful afterwards:
**architecture**, **translation**, **billing**, **mobile**. Each section below is
written for the moment you're about to skip it.
...
### 4.1 Responsive or a separate view?
...
### 4.3 Hover is not available, and it does not fail loudly
...
### 4.4 Viewport, safe areas, and the address bar
...
### 4.5 Touch targets and long-press
...
### 4.6 Sticky headers earn their place on mobile
...
4. **Mobile** — layout vs pointer detection kept distinct, every hover has a
   defined touch behaviour, viewport pinned to the measured height, touch targets
   sized for a thumb.
5. **Verified against the live system**, not just the source — and the check that
