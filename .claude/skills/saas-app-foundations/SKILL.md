...
description: Use when starting a new multi-tenant SaaS web app, or when adding one of its five foundations to an existing one — architecture/stack choice, internationalization, billing with plan limits, multi-user collaboration (roles, sharing, presence), or the mobile version. Front-loads the decisions that are cheap on day one and expensive to retrofit, and carries the failure modes that pass every green check and still ship broken. Written from building Pulse (React + Firebase + Stripe, six languages, three tiers, mobile-first views).
---

# Foundations of a multi-tenant SaaS app

Five things are cheap before the first user and painful afterwards:
**architecture**, **translation**, **billing**, **collaboration**, **mobile**.
Each section below is written for the moment you're about to skip it.
...
5. **Mobile** — layout vs pointer detection kept distinct, every hover has a
   ...
   sized for a thumb.
6. **Verified against the live system**, not just the source — and the check that
