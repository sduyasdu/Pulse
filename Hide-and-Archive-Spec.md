# Beats — Hide & Archive Specification
...
prunes, exports or tiers-down storage for old Beats, and archiving never deletes anything.
...
`entitlements.ts`, not in the Beat itself. An "archived" Beats:
...
2. **"This project is finished — freeze it."** Shared, visible to every member, and the
   ...
   shared Beat takes N manual archives (one per member, uncoordinated), signals nothing,
...
|---|---|---|
| Scope | Your dashboard only | The Beat, for every member |
| Who may | Any member, for themselves | **Owner only** (HA1) |
| Effect | Card moves to your **Hidden** section | Beats becomes **read-only for all members** |
...
A hidden Beat still counts against quota because the quota is an **org-level** count and
hiding is a **per-user** preference. Letting it move the count would be incoherent (whose
hide wins, on a Beat with five members?) and an obvious way to dodge the cap.
...
member belongs with the role that already carries the other Beat-wide consequences — delete,
role changes, member removal (`firestore.rules:175,198,207`). An editor freezing an owner's
Beats out from under them is the wrong default; an owner can always delegate by promoting.
...
- **Not a delete, and not a step toward one.** Nothing is removed, nothing expires, no
  retention clock starts. An archived Beat is complete and readable forever.
...
- **Not a delete guard.** An owner may still delete an archived Beat, and §4.4 makes sure
  the machinery actually permits it — because `Plans-Spec.md` §5.1 makes deleting an archived Beat *the* intended way to reclaim a slot.
...
- **The `archivedAt` mirror on the index entry costs nothing.** The dashboard's self-heal
  loop already fetches each Beat doc to reconcile the denormalized name
...
(When Teams lands and team Beats are listed via a filtered `pulses` query —
...
leave an "archived" Beats with fully editable tasks. Only three call sites actually route
...
| `presence` | self-write (`:245`) | **No** | people still read an archived Beat |
...
### 4.2 `isPulseActive`, and the Beat doc's free read
...
Inside `match /pulses/{pulseId}` **for the Beat doc itself**, don't call this — `resource.data`
...
- **Pinning both fields in branch (b)** means the ordinary edit path can never touch archive
  state even while the Beat is active — the only way in or out is branch (a).
- **`create` refuses a born-archived Beat**, so `duplicatePulse` (`pulses.ts:68-137`) can't
  copy the state forward by accident: a duplicate of an archived Beat is always active (§6).

### 4.4 Deleting an archived Beat must keep working

This is the trap. `deletePulse` (`pulses.ts:213-229`) is a **client-side cascade**: it
deletes `invites`/`epics`/`features`/`resources`, then the Beat doc, then `pulseMembers`
last. Those subcollection deletes go through the same edit gates the freeze is about to
close — so a naive freeze makes an archived Beat **undeletable**, and `Plans-Spec.md` §5.1
depends on deleting an archived Beat being "an easy, obvious path" to reclaim a slot. The
...
This grants nothing new: an owner can already delete the entire Beat and everything in it
...
`onPulseDelete` (SF6, `functions/src/cascade.ts:30-47`, deployed) fires on the Beat doc's
...
has to **reach** the Beat-doc delete, and it deletes subcollections first
(`pulses.ts:245-256`). Without the exemption, the very first `epics` delete throws, the
function aborts before the Beat doc is touched, SF6 never fires, and the Beat survives.
The exemption is what lets an archived Beat be deleted at all.

Alternative considered and rejected: *unarchive-then-delete* in the client — a non-atomic
multi-step flow that leaves the Beat unfrozen for everyone if the delete fails midway.

(A cleaner future refactor: delete the Beat doc **first** and let SF6 own the rest, since
...
- **Presence heartbeats** (`:243-246`) — people still open and read archived Beats.
...
the Beat doc is frozen (§4.3 branch b), the link also can't be rotated or revoked while
archived — so "inert until unarchived" is the only coherent state, and the Share UI says so
rather than appearing to work.

Note the ordering consequence: an owner who wants to add someone to a finished Beat
...
rather than re-counted. So the freeze costs **one additional distinct document** — the Beat
...
Archive folds in **there**, and nowhere else. An archived Beat resolves to
...
### 5.2 In-Beat surfaces

Read-only has to be *explained*, not just enforced, or it reads as a bug:

- **A persistent banner** at the top of the Beat — *"This Beat is archived. Unarchive it to
...
- **Live transition.** The Beat doc is already a live subscription (`pulseStore.ts:108-122`),
  so a member with the Beat open when an owner archives sees the banner appear and the
  controls disable without a reload. The reverse holds for unarchive.

### 5.3 Dashboard

Four sections, in order: **Your Beats**, **Shared with me**, **Archived**, **Hidden**
...
- `hidden` wins over `archivedAt`. Hiding is the user's explicit "get this off my dashboard",
  and a hidden Beat that reappeared in an Archived section would defeat it.
- Otherwise `archivedAt != null` → **Archived**; else owner → **Your Beats**, non-owner →
...
- **Menu:** `Hide` / `Unhide` for everyone; `Archive` / `Unarchive` **for owners only**
  ...
  (`:71-95`).
- **Disabled while archived:** Rename and Invite (both are writes to the frozen Beat doc).
...
- **Archiving asks**, naming the consequence: *"Archiving makes this Beat read-only for all
...
- **The delete confirmation is rewritten.** Today it offers archiving as the gentle
  ...
  right one of the two: *"This erases the Beat and all its data for everyone — it can't be
...
- **Nothing anywhere may imply archiving frees a slot** (§7). Where the plan-limit message
  ...
  `Plans-Spec.md` §5.1 — with deleting an *archived* Beat called out as the natural one.
...
Two different mechanisms can make a Beat read-only: this one, and the over-limit lock on a
...
right now with one click, whereas the plan lock clears only by deleting another Beat or
upgrading. If both apply, show the archived banner; unarchiving then reveals the plan banner,
which is honest — the Beat really is still locked, for a different reason. `pulseLock()`
...
from a Beat page*: it needs the org's Beats ordered by `createdAt`, and there is no `list`
...
set counts **archived Beats too** (PL12/HA8), and the affordance is **delete or upgrade** —
never "archive something to make room".

### 5.7 The last owner may not leave (HA10)

**Invariant: a Beat always has at least one owner.** The last owner cannot remove
...
unarchive, delete and promote all require a live owner; a Beat that loses its last one is
stuck in whatever state it was in, and an *archived* ownerless Beat is frozen and
...
> **You're the only owner, so you can't leave this Beat.**
...
over, and an owner cannot finish deleting their own Beat. It can't be used to strand
anything, because it only opens once the Beat itself is gone.

**The consequence to accept:** this also stops a *non-last* owner self-serving a leave. In a
two-owner Beat, an owner who wants out either self-demotes to editor first (permitted by the
...
| Area | Interaction |
|---|---|
| **Undo/redo** (`Undo-Spec.md`) | Undo entry points are already gated on `canEdit` (`PulsePage.tsx:131-148`), so they disable with the freeze. The in-memory stack is per-Beat and dropped on navigation (`PulsePage.tsx:89-92`) — nothing to purge. **Archiving is not itself undoable**: it's a Beat-lifecycle action, outside the content command stack, and it is confirmed instead (§5.4). |
...
| **Costs** (`Costs-Spec.md`) | `costs` writes freeze (§4.1); `rates` **reads must not** — the `allow read, write` pair (`firestore.rules:293-295`) has to be split so an admin can still open the cost view of a finished project. Cost *reporting* on archived Beats is the point of keeping them. |
...
| **Duplicate** | Duplicating an archived Beat produces an **active** copy — `duplicatePulse` builds a fresh doc (`pulses.ts:68-80`) and the create rule refuses a born-archived one (§4.3). This is deliberate: "start again from the finished thing" is a real workflow, and it doesn't touch the frozen original. Duplicate therefore stays enabled in the card menu while archived. |
| **Teams** (`Collaboration-Spec.md` §3.2) | When team membership cascades access, the freeze sits *outside* the role union: `isPulseActive` ANDs with whatever the effective role resolves to, so a team editor is frozen exactly like a per-Beat editor. Team-Beat listing will want the composite index noted in §3. |
| **BYOS** (`Storage-Spec.md`) | Unrelated to `_Archive/{YYYY-MM}/`, which is where *deleted* task folders go in the user's Drive. Never use the word "archive" for that in user-facing copy; and since archiving freezes task mutations, the reconciler simply has nothing to do on an archived Beat. |
| **Ownerless Beats** | Ruled out by the always-an-owner invariant (**§5.7 / HA10**): the last owner may not leave — they archive, delete, or transfer. Every owner-gated action in this spec depends on that, since an archived Beat with no owner would be frozen, undeletable, and still holding a `maxPulses` slot (§7) forever. Enforced exactly in the client (`CollaboratorsDialog.tsx:38`) and approximately in rules (no owner may self-delete — rules can't count owners). |

## 7. Quota

Owned by `Plans-Spec.md` **PL12** and §3.2; restated because the UI must never contradict it:

**"Beats per org" counts every Beat the org holds — archived and hidden alike.** The quota
...
- **Unarchive needs no quota check.** It can't raise the count, so it can never take an org
  over its cap — which is precisely why counting archived Beats is also the *simpler* rule.
...
every Beat anyone had tidied away.
...
as the freeze**, or a user who archived ten Beats last week gets a release note about
read-only Beats and reasonably assumes theirs are frozen. Phase 1 (§11) is the rename alone.
...
| `dashboard.archiveDetail` | Makes this Beat read-only for all {n} members. Any owner can unarchive it. | new |
...
| `pulse.archivedBanner` | This Beat is archived. Unarchive it to make changes. | owner variant |
| `pulse.archivedBannerMember` | This Beat is archived. Ask an owner to unarchive it to make changes. | non-owner |
...
| `pulse.frozenNotice` | This Beat is archived — changes are turned off. | swallowed-edit toast |
| `collab.soleOwner` | You're the only owner, so you can't leave this Beat. | **rewritten** — was "Sole owner — transfer to leave" (§5.7) |
...
10. `get()` accounting: a feature update by an editor on an archived Beat stays within
...
12. **Tear-down still completes:** the existing deletion-ordering regressions
    (`rules/security.test.ts`, "Beats deletion ordering") must stay green — an owner
    deleting their own Beat walks the full cascade including their own membership doc.
    The owner half of that lockout hazard is now unreachable (they can't self-delete while
    the Beat exists), so that case is rewritten around an editor.
...
- Archiving anything smaller than a Beat (an epic, a task).
...
- Changing how Beats deletion is orchestrated. The server-side cascade already exists and is
  deployed (SF6, §4.4); reordering the client pass around it is a separate change.
- Any change to who can *read* a Beat.

## 13. Decisions (HA1–HA10)

1. **HA1 — Who may archive? ✅ Owner only.** It's a Beat-wide consequence, so it sits with
...
3. **HA3 — Do existing join links still work? ✅ No — they go inert.** A link circulating
   ...
   unactionably. It can't be rotated or revoked while the Beat doc is frozen either, so
   inert is the only coherent state. Adding someone means unarchive → add → re-archive.
4. **HA4 — Can an archived Beat still be deleted? ✅ Yes, and the rules must actively
   permit it.** Owner-delete exemption on every content subcollection (§4.4), because
   `Plans-Spec.md` §5.1 makes deleting an archived Beat the intended way to reclaim a slot.
...
8. **HA8 — Do archived Beats count against `maxPulses`? ✅ Yes** — with hidden ones
...
10. **HA10 — May the last owner leave? ✅ No. Archive or delete instead.** A Beat always
    has at least one owner (§5.7). Every owner-gated action here — archive, unarchive,
    delete, promote — depends on one existing, and an ownerless *archived* Beat would be
